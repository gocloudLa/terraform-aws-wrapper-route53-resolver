locals {
  ...
  resolved_vpc_id = {
    ...
    resolver_key => try(data.aws_vpc.this[resolver_key].id, local.vpc_from_parameters[resolver_key])
  }
  ...
  subnet_name_lookups = {
    ...
    if local.creates_endpoint[resolver_key] && length(local.subnet_ids_raw[resolver_key]) == 0 && length(local.wrapper_private_subnet_ids[resolver_key]) == 0
  }
  ...
  resolved_subnet_ids = {
    ...
    resolver_key => length(local.subnet_ids_raw[resolver_key]) > 0 ? [
      ...
      length(local.wrapper_private_subnet_ids[resolver_key]) > 0 ? local.wrapper_private_subnet_ids[resolver_key] : try(data.aws_subnets.this[resolver_key].ids, [])
    )
  }
  ...
  resolvers = {
    for resolver_key, resolver_config in var.route53_resolver_parameters :
    resolver_key => {
      create_endpoint       = local.creates_endpoint[resolver_key]
      create_security_group = try(resolver_config.create_security_group, var.route53_resolver_defaults.create_security_group, local.creates_endpoint[resolver_key])
      ...
      direction              = upper(try(resolver_config.direction, var.route53_resolver_defaults.direction, "OUTBOUND"))
      ...
      tags = try(resolver_config.tags, var.route53_resolver_defaults.tags, {})
      rules = {
