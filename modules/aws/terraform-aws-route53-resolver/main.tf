resource "aws_security_group" "this" {
  ...
  dynamic "ingress" {
    for_each = var.direction == "INBOUND" && length(var.ingress_cidr_blocks) > 0 ? ["udp", "tcp"] : []
  ...
  lifecycle {
    precondition {
      condition     = var.direction != "INBOUND" || length(var.ingress_cidr_blocks) > 0
      error_message = "INBOUND resolver endpoints require ingress_cidr_blocks when create_security_group is true."
    }
  }
}
...
resource "aws_route53_resolver_rule" "this" {
  ...
  lifecycle {
    precondition {
      condition     = try(each.value.rule_type, "FORWARD") != "FORWARD" || length(try(each.value.target_ips, [])) > 0
      error_message = "FORWARD rule ${each.key} requires at least one target IP."
    }
  }
}
...
resource "aws_ram_resource_share" "this" {
  ...
  name                      = var.ram_name != null ? var.ram_name : "${var.name}-rules"
  allow_external_principals = var.ram_allow_external_principals

  tags = merge(var.tags, { Name = var.ram_name != null ? var.ram_name : "${var.name}-rules" })
