locals {
  ...
  resolver_endpoint_id = try(aws_route53_resolver_endpoint.this[0].id, var.resolver_endpoint_id, null)
  ...
  imported_rule_associations = {
    for k, v in var.rule_associations : k => {
      resolver_rule_id = try(data.aws_route53_resolver_rule.shared[k].id, v.resolver_rule_id, null)
      vpc_id           = v.vpc_id
      name             = try(v.name, k)
    }
  }
